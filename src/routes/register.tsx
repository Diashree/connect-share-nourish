    ...
    <div className="min-h-screen bg-background">
      <div className="container mx-auto px-4 py-10 max-w-3xl">
        <Link to="/" className="inline-flex items-center gap-2">
          <span className="grid h-9 w-9 place-items-center rounded-xl bg-warm-gradient text-primary-foreground"><Sprout className="h-5 w-5" /></span>
          <span className="font-display text-xl font-semibold">FoodConnect<span className="text-secondary">+</span></span>
        </Link>

        <div className="mt-10">
          {step === 1 ? (
            <>
              <h1 className="font-display text-4xl font-semibold tracking-tight text-balance">How will you make impact?</h1>
              <p className="mt-3 text-muted-foreground">Pick the role that best describes you. You can join more roles later.</p>
                ...
                {ROLES.map((r) => {
                  ...
                  return (
                    <button
                      ...
                      className={`text-left card-warm p-6 transition relative ${active ? "ring-2 ring-primary border-primary" : ""}`}
                    >
                      <span className={`grid h-11 w-11 place-items-center rounded-xl ${active ? "bg-primary text-primary-foreground" : "bg-primary-soft text-primary"}`}>
              ...
              {role && role !== "donor" && (
                <div className="mt-6 flex items-start gap-3 rounded-xl bg-amber/10 border border-amber/30 p-4 text-sm">
                  ...
                  <p className="text-muted-foreground">As a {role === "ngo" ? "verified NGO" : "volunteer"}, an admin will review your profile before you can {role === "ngo" ? "claim donations" : "accept tasks"}.</p>
                </div>
              )}
              <div className="mt-8 flex justify-between items-center">
                ...
                <Button disabled={!role} onClick={() => setStep(2)} className="bg-warm-gradient text-primary-foreground hover:opacity-90 h-11 px-6">
            ...
            <>
              <button onClick={() => setStep(1)} className="inline-flex items-center text-sm text-muted-foreground hover:text-foreground">
                <ArrowLeft className="h-4 w-4 mr-1" />Back
              </button>
              <h1 className="mt-4 font-display text-4xl font-semibold tracking-tight">Tell us about yourself</h1>
              <p className="mt-2 text-muted-foreground">You're joining as <span className="font-medium text-foreground capitalize">{role}</span>.</p>
              <form onSubmit={submit} className="mt-8 space-y-4 max-w-xl">
                ...
                <div>
                  ...
                  <Input id="password" type="password" required minLength={6} value={form.password} onChange={(e) => set("password", e.target.value)} className="mt-1.5" />
                </div>
                {role === "ngo" && (
                  <div>
                    <Label htmlFor="org">Organization name</Label>
                    <Input id="org" required value={form.org} onChange={(e) => set("org", e.target.value)} className="mt-1.5" />
                  </div>
                )}
                <div>
                ...
                {role !== "donor" && (
                  <div>
                    ...
                    <Textarea id="bio" value={form.bio} onChange={(e) => set("bio", e.target.value)} className="mt-1.5" rows={3} />
                  </div>
                )}
